<?php
/**
 * proses_ubah_simpanan.php (VERSI MANUAL ID TRANSAKSI)
 * - Membuat id_transaksi dari id terbesar + 1
 * - Format: TRX-WJB-0000123, dst sesuai jenis_simpanan
 * - Periode dihitung berdasarkan tanggal_join & jatuh tempo
 * - Bukti transaksi disimpan ke folder 'bukti_bayar/'
 */
...
function save_bukti(string $fieldName, string $idTransaksi, string $jenisTransaksi): ?string {
  ...
  $f = $_FILES[$fieldName];
  if ($f['error'] !== UPLOAD_ERR_OK) throw new RuntimeException('Upload bukti gagal (kode '.$f['error'].')');
  if ($f['size'] > 5*1024*1024) throw new RuntimeException('Ukuran bukti maksimal 5MB.');

  $ext = strtolower(pathinfo($f['name'], PATHINFO_EXTENSION));
  $allowed = ['jpg','jpeg','png','webp','pdf'];
  if (!in_array($ext, $allowed, true)) throw new RuntimeException('Format bukti harus jpg/jpeg/png/webp/pdf.');

  $destDir = __DIR__ . DIRECTORY_SEPARATOR . 'bukti_bayar';
  if (!is_dir($destDir) && !mkdir($destDir, 0775, true)) throw new RuntimeException('Gagal membuat folder bukti_bayar.');

  $base = strtolower($jenisTransaksi) . '-' . $idTransaksi . '.' . $ext;
  $destAbs = $destDir . DIRECTORY_SEPARATOR . $base;
  if (!move_uploaded_file($f['tmp_name'], $destAbs)) throw new RuntimeException('Gagal menyimpan bukti ke server.');
...
try {
  $stmt = $conn->prepare("SELECT id, simpanan_wajib, saldo_sukarela, saldo_total, tanggal_join FROM anggota WHERE id=? FOR UPDATE");
  $stmt->bind_param('i', $id_anggota);
  $stmt->execute();
  $res = $stmt->get_result();
  if ($res->num_rows === 0) throw new Exception('Anggota tidak ditemukan.');
  $agt = $res->fetch_assoc();
  $stmt->close();
