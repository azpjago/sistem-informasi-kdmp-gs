<?php
// proses_pendaftaran.php (VERSI FINAL DENGAN ID TRANSAKSI GLOBAL)
header('Content-Type: application/json');
date_default_timezone_set('Asia/Jakarta');

function handleUpload($file, $folder, $prefix = '') {
    if (!$file || $file['error'] !== UPLOAD_ERR_OK) return ['error' => 'File tidak valid. Code: ' . ($file['error'] ?? 'N/A')];
    if ($file['size'] > 5 * 1024 * 1024) return ['error' => 'Ukuran file maks 5MB.'];
    if (!is_dir($folder) && !mkdir($folder, 0777, true)) return ['error' => 'Gagal membuat direktori.'];
    $extension = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    $fileName = ($prefix ? $prefix . '_' : '') . uniqid() . '_' . time() . '.' . $extension;
    $targetPath = $folder . '/' . $fileName;
    if (move_uploaded_file($file['tmp_name'], $targetPath)) return ['success' => true, 'path' => $targetPath];
    return ['error' => 'Gagal memindahkan file. Periksa izin folder.'];
}

function getTransactionPrefix($jenis_simpanan) {
    switch ($jenis_simpanan) {
        case 'Simpanan Pokok': return 'TRX-PKK';
        case 'Simpanan Wajib': return 'TRX-WJB';
        case 'Simpanan Sukarela': return 'TRX-SKR';
        default: return 'TRX-OTH';
    }
}
...
try {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        if (preg_match('/(\d{4})$/', $lastNo, $matches)) $lastNum = (int) $matches[1];
    }
    ...
    if (!$stmt_anggota->execute()) throw new Exception("Gagal menyimpan data anggota: " . $stmt_anggota->error);
    $anggota_id = $conn->insert_id;

    // Siapkan statement untuk pembayaran
    $stmt_pembayaran = $conn->prepare("INSERT INTO pembayaran (anggota_id, id_transaksi, jenis_simpanan, jenis_transaksi, jumlah, nama_anggota, bulan_periode, metode, bukti, status, tanggal_bayar) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
    ...
    $pembayaran_data = [];
    if ($simpanan_pokok > 0) $pembayaran_data[] = ['jenis_simpanan' => 'Simpanan Pokok', 'jumlah' => $simpanan_pokok, 'status' => 'Simpanan Pokok ' . $nama . ' - ' . $bulan_daftar_txt_info];
    if ($simpanan_wajib_amount > 0) $pembayaran_data[] = ['jenis_simpanan' => 'Simpanan Wajib', 'jumlah' => $simpanan_wajib_amount, 'status' => 'Pembayaran Simpanan Wajib ' . $bulan_daftar_txt_info];
    if ($saldo_sukarela > 0) $pembayaran_data[] = ['jenis_simpanan' => 'Simpanan Sukarela', 'jumlah' => $saldo_sukarela, 'status' => 'Setor Simpanan Sukarela Sebesar ' . number_format($saldo_sukarela) . ' - ' . $bulan_daftar_txt_info];
    ...
    if (isset($stmt_anggota)) $stmt_anggota->close();
